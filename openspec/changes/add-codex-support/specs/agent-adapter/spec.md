...
- **THEN** 接口 SHALL 包含以下抽象方法：
  - `agent_type` 属性：返回代理标识符（如 `'claude'`、`'codex'`）
  - `resolve_command(command_name)`: 解析 agent 命令字符串
...
- **THEN** 生成命令字符串包含 `codex exec resume {session_id} --json --cd {project_dir} {prompt}`
...
- **WHEN** stdout 输出首条 JSONL 事件 `{"type":"thread.started","thread_id":"xxx"}`
- **THEN** 系统解析 `thread_id` 字段
- **AND** 将 session store 中的临时 session ID 替换为真实 `thread_id`

#### Scenario: 捕获失败 graceful fallback

- **GIVEN** stdout 首行不是有效的 `thread.started` 事件
- **WHEN** 系统尝试解析 session ID
- **THEN** 保留原始临时 session ID
- **AND** 记录警告日志
...
- **WHEN** hook-router.sh 接收 stdin JSON
- **THEN** JSON 字段名可能与 Claude 不同
- **AND** hook-router.sh 做字段名规范化（如有差异）
- **AND** 正常分发到对应 hook 脚本
