...
- **AND** 转发到 Callback 后端 `/cb/agent/new` 接口
...
`/new` 指令 SHALL 支持 `--dir=` 参数指定工作目录、`--cmd=` 参数指定 Agent Command，同时也支持交互式目录选择。
...
Callback 后端 SHALL 提供 `/cb/agent/new` 端点，接收并处理新建会话请求，支持 Claude 和 Codex 两种 agent 类型。

#### Scenario: 执行新建会话（Claude）

- **GIVEN** 请求的 `agent_type` 为 `claude`
- **AND** 异步线程启动
- **WHEN** 执行 agent 命令
...
### Requirement: Agent Command 多命令配置
...
`/new` 指令 SHALL 支持 `--cmd` 参数，用于从预配置列表中选择 Agent Command。
...
当配置了 Agent Command 时，新建会话卡片 SHALL 包含 Command 选择下拉框。

#### Scenario: 显示 Command 选择器

- **GIVEN** `CLAUDE_COMMAND` 或 `CODEX_COMMAND` 已配置（单命令或多命令）
- **AND** 用户发送 `/new`（触发卡片）
- **WHEN** 构建目录选择卡片
- **THEN** 卡片包含 `select_static` 下拉框（name 为 `agent_command`）
- **AND** 选项包含所有配置的命令
- **AND** 默认选中第一个命令
...
- **AND** 用户点击"创建会话"按钮
- **WHEN** 处理表单提交
- **THEN** 将选中的 `command` 传递到 Callback 后端 `/cb/agent/new`

### Requirement: /reply 指令

系统 SHALL 支持 `/reply` 指令，允许用户在回复消息时指定 Agent Command。

#### Scenario: 基本 /reply 格式

- **GIVEN** 用户回复某条 Agent 会话消息
- **AND** 消息内容为 `/reply 继续帮我完善`
- **WHEN** 网关解析指令
- **THEN** 识别为 `/reply` 命令
- **AND** 提取 `prompt` 为 `继续帮我完善`
- **AND** 从 parent_id 映射中获取 session 信息
- **AND** 使用 session 记录的 Agent Command 继续会话
...
- **THEN** 系统在 `DEFAULT_CHAT_DIR` 目录下创建新 Agent 会话，消息内容作为 prompt
- **AND** 该会话成为活跃的默认会话
...
- **THEN** 系统在 `DEFAULT_CHAT_DIR` 目录下创建新 Agent 会话
- **AND** 该会话替换为新的活跃默认会话
