...
def _execute_and_check(session_id: str, project_dir: str, prompt: str, chat_id: str = '',
    ...
    try:
        ...
        # 在后台等待完成
        _run_in_background(_wait_for_completion, (proc, session_id, chat_id, message_id))
        return Response.processing()


def _wait_for_completion(proc: subprocess.Popen, session_id: str,
    ...
    try:
        ...
        if proc.returncode == 0:
            ...
            if chat_id and stderr:
                _send_error_notification(chat_id, message_id, stderr.strip()[:MAX_LOG_LENGTH])
    except subprocess.TimeoutExpired:
        ...
        if chat_id:
            _send_error_notification(chat_id, message_id, str(e)[:MAX_NOTIFICATION_LENGTH])
...
def _send_error_notification(chat_id: str, message_id: str, error_msg: str):
    ...
    text = f"❌ Claude 执行异常:\n{error_msg}"
    success, result = reply_feishu_text(chat_id, text, message_id)
    if success:
