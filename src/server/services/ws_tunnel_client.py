...
class WSTunnelClient:
    ...
    def __init__(self, gateway_url: str, owner_id: str,
                 binding_params: Optional[Dict[str, Any]] = None) -> None:
        """
        Args:
            gateway_url: 网关 HTTP base URL（如 http://gateway:8080）
            owner_id: 飞书用户 ID
            binding_params: per-user 配置参数（传递给 gateway BindingStore）
        """
        self.gateway_url = gateway_url
        self.owner_id = owner_id
        self.binding_params = binding_params or {}
    ...
    def _connect_and_run(self) -> None:
        ...
        register_data.update(self.binding_params)
...
def start_ws_tunnel_client(gateway_url: str, owner_id: str,
                           binding_params: Optional[Dict[str, Any]] = None) -> WSTunnelClient:
    """启动 WebSocket 隧道客户端
